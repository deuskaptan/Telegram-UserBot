...
async def bot_ver(event):
    ...
    if which("git") is not None:
        ...
        await event.edit(
            "Bu arada DeuS seni çok seviyor. ❤"
        )
...
async def pipcheck(pip):
    ...
    if pipmodule:
        ...
        await pip.edit("`Bir örnek görmek için .deus pip komutunu kullanın.`")


@register(outgoing=True, pattern="^.alive$")
async def amialive(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("`Merhaba 💋 DeusUserBot Çalışıyor... Tepki amaçlı yapıldı ✨.`")
...
CMD_HELP.update({
    "alive": ".alive\
    \nKullanım: Deus botunun çalışıp çalışmadığını kontrol etmek için kullanılır."
})
